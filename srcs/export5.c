...
{
	if ((!ft_strncmp(info->tokens[info->i].args[*a], "PWD\0", 4)
			&& ft_strlen(info->tokens[info->i].args[*a]) == 3 && !info->pwd)
		|| (!ft_strncmp(info->tokens[info->i].args[*a], "OLDPWD\0", 7)
			&& ft_strlen(info->tokens[info->i].args[*a]) == 6 && !info->oldpwd))
	{
